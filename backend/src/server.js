...
async function getDesignPricingCached(designId, ttlMs = 3600000) {
...
async function getDesignMetaCached(designId, ttlMs = 3600000) {
...
app.get("/health", async (_req, res) => {
  try {
    ...
    // Frontend expects: {status: "healthy", database: {status: "connected"}}
    res.json({ 
      status: "healthy",
      ok: true, 
...
app.post("/webhooks/aurora", express.json(), (req, res) => {
  console.log('\n========== AURORA WEBHOOK RECEIVED ==========');
  console.log('Timestamp:', new Date().toISOString());
  console.log('Headers:', JSON.stringify(req.headers, null, 2));
  console.log('Body:', JSON.stringify(req.body, null, 2));
  console.log('===========================================\n');
  
  // Respond immediately to Aurora so they know we received it
  ...
  try {
    ...
    const items = await Promise.all(
      proj.items.map(async (p) => {
        ...
        // Company requested: Drop domestic calculation function - keeping pricing only
        ...
        return {
          ...
          systemSize: sizeKw,
          totalCost: priceVal,
          domesticContent: null,  // REMOVED per company request
          feocCompliant: null,    // REMOVED per company request
...
app.get("/compare/:designId", async (req, res) => {
  try {
    ...
    // Company requested: Drop domestic calculation function - keeping pricing only
    // REMOVED: FEOC evaluation per company request
...
// ---------- FEOC evaluate (foreign content only, 2026 compliance) ----------
    ...
    const items = await Promise.all(
      proj.items.map(async (p) => {
        ...
        if (firstDesign) {
          ...
          // Build ALL parts with complete data for FEOC Calculator template
          if (Array.isArray(cmp?.items)) {
            keyParts = cmp.items
              .map((x) => ({
                name: x.name || x.type || 'Unknown',
                type: x.type || 'component',
                manufacturer: x.manufacturer || 'Unknown',
                sku: x.sku || x.matched_sku || 'N/A',
                matched_sku: x.matched_sku || x.sku || null,
                quantity: x.quantity || 1,
                count: x.quantity || 1, // Legacy field for backwards compatibility
                unit_price: x.unit_price || 0,
                line_total: x.line_total || 0,
                origin_country: x.origin_country || 'UNKNOWN',
                is_domestic: x.origin_country === 'US',
                domestic: x.origin_country === 'US' // Legacy field for backwards compatibility
              }))
              .filter((x) => x.name);
          }
        }
        ...
        // Company requested: Drop domestic calculation function - keeping pricing only
        const domesticPct = null;
        
        // Calculate FEOC compliance based on actual parts
        const FEOC_COUNTRIES = ['CN', 'RU', 'KP', 'IR'];
        let feoc = null;
        if (Array.isArray(cmpItems) && cmpItems.length > 0) {
          const hasFEOCParts = cmpItems.some(item => 
            FEOC_COUNTRIES.includes(item.origin_country)
          );
          feoc = !hasFEOCParts; // true if no FEOC parts, false if FEOC parts found
        }
        ...
        return {
          ...
          complianceDetails: totals ? {
            ...
            feocCompliant: null,
            steelIronCompliant: null,
            manufacturedProductsCompliant: null,
            requiredDomestic: null,
            projectEligible: null,
            smallProject: null,
            earlyConstruction: null,
            prevailingWageCompliant: null,
            feocBreakdown: null,
          } : null,
        };
      })
...
// Auto-detect email provider SMTP settings
function getEmailConfig() {
  const emailFrom = process.env.EMAIL_FROM;
  const emailPassword = process.env.EMAIL_PASSWORD;
  
  if (!emailFrom || !emailPassword || emailFrom.includes('your-email')) {
    return null;
  }
...
// Email endpoint for FEOC reports
app.post('/api/send-feoc-report', async (req, res) => {
  try {
    ...
    if (!emailConfig) {
      console.log('📧 Email not configured - add EMAIL_FROM and EMAIL_PASSWORD to .env');
      return res.json({ 
        success: true, 
        message: 'Email simulation - configure EMAIL_FROM and EMAIL_PASSWORD in root .env to send real emails'
      });
    }
    ...
    try {
      ...
      const emailSubject = subject || `FEOC Report - ${reportData?.projectData?.projectName || 'Project'}`;
      const emailBody = `
${message || 'Please find the attached FEOC compliance report.'}

PROJECT DETAILS:
• Project: ${reportData?.projectData?.projectName || 'N/A'}
• Customer: ${reportData?.projectData?.customerName || 'N/A'}
• System Size: ${reportData?.projectData?.totalSystemSize || 'N/A'} kW
• Generated: ${new Date().toLocaleString()}

${reportData?.feocResults ? `
FEOC COMPLIANCE:
• Steel/Iron: ${reportData.feocResults.steelIronDomesticPercentage?.toFixed(1) || 'N/A'}%
• Manufactured: ${reportData.feocResults.manufacturedProductsDomesticPercentage?.toFixed(1) || 'N/A'}%
• Compliant: ${reportData.feocResults.domesticContentCompliant ? 'YES' : 'NO'}
` : ''}

---
IRS Domestic Content Bonus Calculator
      `.trim();
      ...
      res.status(500).json({ 
        ...
        hint: 'Gmail users: Use App Password from security settings'
      });
    }
  } catch (error) {
